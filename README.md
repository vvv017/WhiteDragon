...
- Local-only avatar image storage through IndexedDB
...
The small root installer fetches the readable source parts from this repository once per WhiteDragon version and caches the assembled source locally.
...
WhiteDragon runs locally after loading its source.
...
The readable source is split across `src/parts/part-*.part`. The root `whitedragon.user.js` is the installable loader and caches the assembled source per version. The included GitHub Actions workflow can also concatenate the parts into a standalone script for validation.
